...
/**
 * 🔥 NEW: how long before a member is considered "dead"
 * prevents random host deletion due to brief disconnects
 */
export const PARTY_MEMBER_TIMEOUT_MS = 45 * 1000; // 45s
...
/**
 * 🔥 FIXED: Safe leave logic (host won't be deleted accidentally)
 */
export async function leaveParty(code, userId) {
  ...
  // 🔥 CRITICAL FIX: Only delete party if NO members left
  const membersSnap = await get(ref(db, `parties/${code}/members`));
  ...
  // 🔥 If host leaves → transfer host instead of deleting party
  if (hostId === userId) {
    const nextHostId = Object.keys(members)[0];
    ...
    if (nextHostId) {
      ...
      await update(ref(db, `parties/${code}/members/${nextHostId}`), {
        isHost: true,
      });
    }
  } else {
...
export function subscribeToMembers(code, callback) {
  ...
  const handler = (snapshot) => {
    ...
    // 🔥 FIX: remove stale members safely
...
/**
 * 🔥 CRITICAL: KEEP DISABLED
 */
export function leavePartyOnUnload() {
  // intentionally empty to prevent host kicks on refresh
}