...
export async function generateUniquePartyCode(maxAttempts = 25) {
...
export function leavePartyOnUnload(code, userId, isHost) {
  if (!code || !userId) return;

  try {
    if (isHost) {
      remove(ref(db, `parties/${code}`));
    } else {
      remove(ref(db, `parties/${code}/members/${userId}`));
    }
  } catch {
    // best effort
  }
